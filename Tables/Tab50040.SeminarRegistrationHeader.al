Table 50040 "Seminar Registration Header"
{
    ...
    Permissions = tabledata "Seminar Registration Line" = rm;
    ...
    fields
    {
        ...
        field(19; "Seminar Price"; Decimal)
        {
            ...
            trigger OnValidate()
            var
                Line: Record "Seminar Registration Line";
            begin
                if Status <> Status::Cancelled then begin
                    if Confirm(UpdateMessage) then begin
                        Line.Reset();
                        Line.SetFilter(Line."Seminar Registration No.", "No.");
                        if Line.FindSet() then begin
                            repeat
                                if Line.Registered = false then begin
                                    Line.Validate(Line."Seminar Price", "Seminar Price");
                                    Line.Modify(true);
                                    CalcFields(Amount);
                                end;
                            until Line.Next() = 0;
    ...
    var
        ...
        CanNotBeModifiedErr: label 'Starting date cannot be changed';
        UpdateMessage: label 'Do you want to update?';
        ModificationCanceled: label 'Update failed';
}
